...
def calculate_dead_loss(df: pd.DataFrame, timeframe: int, bins: int, nodeID:int) -> pd.DataFrame:
    # add new column for date only
    df.to_csv("deadloss_original.csv")
    ...
    df_send_raw.to_csv("deadloss_sendonly.csv")
    df_recv_raw.to_csv("deadloss_receiveonly.csv")
    ...
    timeframe=50 # to remove later
    df_joined['deadloss'] = np.where((df_joined['delay'] > timeframe), 1, 0)
    ...
    df_joined.to_csv(f'deadloss_raw_node{nodeID}.csv')
    ...
    df_joined.to_csv(f'deadloss_rawWithBin_node{nodeID}.csv')
    ...
    metric.to_csv(f'deadloss_metric_node{nodeID}.csv')
    return metric

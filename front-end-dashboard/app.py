...
app = dash.Dash(
    __name__, use_pages=True, server=server, external_stylesheets=[dbc.themes.DARKLY, dbc.icons.BOOTSTRAP]
)
...
app.layout = html.Div(children=
    [        
        navbar,
        html.Div(children=dash.page_container, className='page-container')
    ]
)
...
def data_scheduler(n_intervals):
    ...
    if type(df_pdr) == type(None):
        ...
        pdr_graph = px.line(df_pdr, x='env_timestamp', y=['failed_packets_precentage', 'successful_packets_precentage'], color_discrete_sequence=['red', 'blue'], title='Packet Delivery Ratios', labels={'env_timestamp': 'Time Invervals', 'value': 'Percentage'})
        
    if type(df_icmp) == type(None):
        icmp_graph = px.bar(title='ICMP Packets')
    else:
        icmp_graph = px.bar(df_icmp, x='node', y='sub_type_value', title='ICMP Packets', labels={'node': 'Node ID', 'sub_type_value': 'Total ICMP Packets'})
